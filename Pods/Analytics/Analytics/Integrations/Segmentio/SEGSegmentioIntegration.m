...
- (id)initWithConfiguration:(SEGAnalyticsConfiguration *)configuration
{
    if (self = [super init]) {
        ...
        // self.apiURL = [NSURL URLWithString:@"https://api.infinario.com/segment"];
         // self.apiURL = [NSURL URLWithString:@"www.st.fmph.uniba.sk/~sulik12/projekty/infinario"];
         self.apiURL = [NSURL URLWithString:@"http://devapi.infinario.com/segment"];
        ...
        self.settings = @{ @"token" : configuration.token };
        [self validate];
...
{
    return [NSString stringWithFormat:@"<%p:%@, %@>", self, self.class, [self.configuration dictionaryWithValuesForKeys:@[ @"token" ]]];
}
...
- (void)registerForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken options:(NSDictionary *)options
{
    ...
    [self.context[@"device"] setObject:[token copy] forKey:@"project_token"];
}
...
- (void)flushWithMaxSize:(NSUInteger)maxBatchSize
{
    [self dispatchBackground:^{
      ...
      [payloadDictionary setObject:self.configuration.token forKey:@"token"];
...
- (void)sendData:(NSData *)data
{
    ...
    NSData *authData = [[NSString stringWithFormat:@"%@:", self.configuration.token] dataUsingEncoding:NSUTF8StringEncoding];
    NSString *auth64baseString = [authData base64EncodedStringWithOptions:nil];
//    SEGLog(auth64baseString);
//        SEGLog(@"=========================");
    [urlRequest setValue:[NSString stringWithFormat:@"Basic %@", auth64baseString] forHTTPHeaderField:@"Authorization"];
    [urlRequest setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
    [urlRequest setHTTPMethod:@"POST"];
    [urlRequest setHTTPBody:data];
//        SEGLog(@"jozooooo %@, %@", urlRequest, [NSString stringWithFormat:@"Basic %@", self.configuration.token]);
// OAuth
                                                 ...
                                                 completion:^{
                                                   [self dispatchBackground:^{
                                                     if (self.request.error) {
                                                         SEGLog(@"%@ API request on %@ had an error: %@", self,self.request.urlRequest, self.request.error);
