...
+ (instancetype)configurationWithToken:(NSString *)token
{
...
{
    return [NSString stringWithFormat:@"<%p:%@, %@>", self, self.class, [self dictionaryWithValuesForKeys:@[ @"token", @"shouldUseLocationServices", @"flushAt" ]]];
}
...
- (void)refreshSettings
{
    ...
    NSMutableURLRequest *urlRequest = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"https://cdn.segment.com/v1/projects/%@/settings", self.configuration.token]]];
...
{
    [self setupWithConfiguration:[SEGAnalyticsConfiguration configurationWithToken:token]];
}

- (id)initWithToken:(NSString *)token
{
    return [self initWithConfiguration:[SEGAnalyticsConfiguration configurationWithToken:token]];
}
